...
class PhysicalTrackSpringWA():
    
#     allowable_keys = ('obj_mass', 'obj_fric', 'tstep', 'duration', 'failure_error',
#                       'f_bound', 'max_ref', 'max_freq')

    # internal methods
    def __init__(self, env_id=None, seed_=None, spring_k=0.1, config_file=None, **kwargs):
    ...
    def _update_state(self, net_f, net_df, t, tstep):
        ...
        self.a = (net_f - self.obj_fric*self.v) / self.obj_mass # newly added in env_spring_wa
        obj_state = np.array([self.x, self.v, net_f])
        self.x, self.v, self.net_f = rk4_step(self._dynamic_sys, obj_state, net_df, t, tstep)
        
#         if hasattr(self.x, "__len__"):
#             print('t, self.x = ', t, self.x)
#             print('self.v = ', self.v)
#             print('self.net_f = ', self.net_f)
#             raise ValueError('1: env.x is messed up!')
...
#         return initial observations
        ...
        if max_freq=='max':
            ...
            traj_max_f = self.max_freq*np.random.rand()#choice([0.01, 0.4, 0.5])
...
#         return initial observations
        ...
        if max_freq=='max':
            ...
            traj_max_f = np.random.choice([0.01, 0.4, 0.5])
        elif isinstance(max_freq, (float)):
            traj_max_f = max_freq
        else:
            raise ValueError
...
#         self.done = self.is_terminal() # Check terminal due to new positional error 
        ...
        return [r, r_d, self.x, self.v, fn, fn_dot], self.reward(r,self.x), self.done, None
